...
function MessageContent({ content, isAssistant }: { content: string; isAssistant: boolean }) {
  ...
  const hasArtifact =
    content.includes('<boltArtifact') || content.includes('data-artifact-id');

  return (
    <div>
      {cleanContent && <span className="whitespace-pre-wrap">{cleanContent}</span>}
      {hasArtifact && (
        <div className="mt-2 px-2.5 py-1.5 bg-[#e8ff5a]/10 border border-[#e8ff5a]/20 rounded-lg flex items-center gap-2">
          ...
          <span className="text-[#e8ff5a] text-xs font-medium">Generated files</span>
        </div>
      )}
    </div>
  );
